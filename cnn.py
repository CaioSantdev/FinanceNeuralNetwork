# ===================== CONFIGURAÇÕES GERAIS PADRONIZADAS =====================
import numpy as np
import pandas as pd
...
# ===================== CONFIGURAÇÕES PADRONIZADAS (IGUAL AO LSTM) =====================
TICKER = "ITUB4.SA"
...
def CalcularIndicadores(df):
    """VERSÃO PADRONIZADA - MESMA DO LSTM"""
    df = df.copy()
    
    # CORREÇÃO: Usar shift(2) para evitar qualquer lookahead bias
    df['Retornos'] = df['close'].pct_change().shift(2)
    
    # Todos os indicadores com shift(2) - PADRONIZADO
    ...
    # ADICIONAIS PARA CNN (mas com mesmo shift)
    df['EMA_12'] = ta.ema(df['close'], length=12).shift(2)
    df['Retorno_5d'] = df['close'].pct_change(5).shift(2)
    
    # Bollinger Bands
    bb = ta.bbands(df['close'], length=20)
    if bb is not None:
        df['BB_upper'] = bb.iloc[:, 0].shift(2)
        df['BB_lower'] = bb.iloc[:, 2].shift(2)
        df['BB_width'] = (df['BB_upper'] - df['BB_lower']) / df['close'].shift(2)
    
    # Volume features
    df['Volume_SMA_20'] = df['volume'].rolling(20).mean().shift(2)
    df['Volume_Ratio'] = (df['volume'] / df['Volume_SMA_20']).shift(2)
...
def BacktestComTaxas(df, datas, previsoes, capital_inicial=CAPITAL_INICIAL):
    ...
    for i, data_atual in enumerate(datas):
        ...
        # CRITÉRIOS DE ENTRADA PADRONIZADOS
        if not trade_ativo and capital > preco_atual:
            ...
            # CRITÉRIOS DE SAÍDA PADRONIZADOS
...
def ValidarPrevisoes(valores_reais, previsoes, datas_teste):
    ...
    plt.plot(datas_teste, previsoes, label='Previsão CNN1D', linewidth=1, alpha=0.7, color='red')
    plt.title(f'Comparação: Preço Real vs Previsão CNN1D ({TICKER})')
    plt.legend()
...
def CriarModeloCNN1D(formato_entrada):
    ...
    modelo = Sequential([
        ...
        Dropout(0.1),
        Dense(1)  # Saída: preço previsto (PADRONIZADO)
    ])
    ...
    print("✅ Modelo CNN1D padronizado criado")
    return modelo

# ===================== EXECUTOR CNN1D PADRONIZADO =====================

def ExecutarCNN1D():
    print(f"\n🎯 INICIANDO MODELO CNN1D PADRONIZADO PARA {TICKER}")
    print("=" * 60)
    
    # Baixar dados (PADRONIZADO)
    dados = BaixarDados(TICKER, "2019-01-01", DATA_FIM_TESTE)
    if dados is None:
        return None

    # Preparar dados (PADRONIZADO)
    df = dados[['Open','High','Low','Close','Volume']].copy()
    df.columns = ['open','high','low','close','volume']
    df = CalcularIndicadores(df)

    # Separar treino e teste (PADRONIZADO)
    ...
    # Verificar separação (PADRONIZADO)
    ...
    # Selecionar características e preparar dados (PADRONIZADO)
    ...
    # Métricas de regressão (PADRONIZADO)
    ...
    # Validar previsões (PADRONIZADO)
    ValidarPrevisoes(valores_reais, previsoes, datas_teste)

    # Backtest com taxas (PADRONIZADO)
    ...
    # Análise de drawdown (PADRONIZADO)
    ...
    # Gráfico final (PADRONIZADO)
    plt.figure(figsize=(12, 6))
    plt.plot(curva_patrimonio, label='CNN1D com Gestão de Risco', linewidth=2, color='green')
    ...
    plt.title(f'Desempenho CNN1D vs Buy & Hold ({TICKER})')
...
if __name__ == "__main__":
    ...
    print("🚀 INICIANDO EXECUÇÃO DO MODELO CNN1D PADRONIZADO")
    print("=" * 70)
    
    resultados = ExecutarCNN1D()
