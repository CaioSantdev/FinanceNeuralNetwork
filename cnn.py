...
print(f"=== CONFIGURAÇÃO CNN1D MELHORADA PARA {TICKER} ===")
...
# ===================== MODELO CNN1D MELHORADO =====================
def CriarModeloCNN1DMelhorado(formato_entrada):
    modelo = Sequential([
    ...
    print("✅ Modelo CNN1D melhorado criado")
    return modelo
...
def BacktestEstrategiaInteligente(df, datas, previsoes_retorno, capital_inicial=CAPITAL_INICIAL):
    ...
    print(f"\n🔍 INICIANDO BACKTEST COM ESTRATÉGIA INTELIGENTE")
    sinais_compra = 0
    trades_executados = 0
    ...
    for i, data_atual in enumerate(datas):
        ...
        if sinal == "COMPRAR" and not trade_ativo and capital > preco_atual:
            ...
            if capital_para_trade > preco_atual:
                ...
                print(f"💰 COMPRA: {data_atual} | Preço: {preco_atual:.2f} | Retorno Previsto: {retorno_previsto:.2%}")
        ...
        # EXECUTAR VENDA
            ...
            print(f"💸 VENDA ({motivo}): {data_atual} | Preço: {preco_atual:.2f} | Retorno Real: {retorno_real:+.2f}%")
    ...
    if trade_ativo and len(datas) > 0:
        ...
        if ultima_data in df.index:
            ...
            print(f"🔚 FECHAMENTO FORÇADO: {ultima_data} | Retorno: {retorno_real:+.2f}%")
    
    print(f"\n📊 RESUMO ESTRATÉGIA INTELIGENTE:")
...
# ===================== VALIDAR PREVISÕES =====================
def ValidarPrevisoesRetorno(valores_reais, previsoes, datas_teste):
    plt.figure(figsize=(12, 6))
    plt.plot(datas_teste, valores_reais, label='Retorno Real 5d', linewidth=2, color='blue')
    plt.plot(datas_teste, previsoes, label='Retorno Previsto CNN1D', linewidth=1, alpha=0.7, color='red')
    plt.title(f'Comparação: Retorno Real vs Previsto CNN1D ({TICKER})')
    plt.legend()
...
# ===================== EXECUTOR CNN1D MELHORADO =====================
def ExecutarCNN1DMelhorado():
    print(f"\n🎯 INICIANDO MODELO CNN1D MELHORADO PARA {TICKER}")
    print("=" * 70)
    ...
    modelo = CriarModeloCNN1DMelhorado((X_treino.shape[1], X_treino.shape[2]))

    print(f"\n🔥 Treinando modelo para prever retornos...")
    ...
    print(f"\n✅ ========== MÉTRICAS DE PREVISÃO DE RETORNO ==========")
    print(f"MAE : {mae:.4f}")
    print(f"RMSE: {rmse:.4f}")
    print(f"R²  : {r2:.4f}")

    # Validar previsões
    ValidarPrevisoesRetorno(retornos_reais, previsoes_retorno, datas_teste)

    # Backtest com estratégia inteligente
    print(f"\n💼 Executando estratégia de trading inteligente...")
    ...
    print(f"\n💰 ========== RESULTADOS ESTRATÉGIA INTELIGENTE ==========")
    ...
    # Gráfico final
    plt.figure(figsize=(12, 6))
    plt.plot(curva_patrimonio, label='CNN1D - Estratégia Inteligente', linewidth=2, color='green')
    ...
    plt.title(f'CNN1D Melhorado vs Buy & Hold ({TICKER})')
...
if __name__ == "__main__":
    ...
    print("🚀 INICIANDO CNN1D MELHORADO - ESTRATÉGIA INTELIGENTE")
    print("=" * 70)
    
    resultados = ExecutarCNN1DMelhorado()
    ...
    if resultados:
        ...
        print(f"💰 Trades Executados: {resultados['trades']['total_trades']}")
    else:
        print(f"\n💥 ❌ Erro na execução")