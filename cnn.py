...
print(f"=== CONFIGURAÇÃO CNN1D PARA {TICKER} ===")
...
def CriarModeloCNN1D(formato_entrada):
    ...
    print("✅ Modelo CNN1D criado com sucesso")
    return modelo
...
def BacktestComTaxas(df, datas, previsoes, capital_inicial=CAPITAL_INICIAL):
    ...
    for i, data_atual in enumerate(datas):
        ...
        # CRITÉRIOS DE ENTRADA FLEXÍVEIS
        if not trade_ativo and capital > preco_atual:
            condicao_previsao = razao_preco > 1.008  # 0.8% acima
            condicao_tendencia = preco_atual > df.loc[data_atual, 'SMA_10']
            condicao_rsi = 30 < df.loc[data_atual, 'RSI_14'] < 70
            
            if condicao_previsao and condicao_tendencia:
                sinal = "COMPRAR"
                sinais_compra_identificados += 1
                
                # DEBUG: Mostrar primeiros sinais
                if sinais_compra_identificados <= 5:
                    print(f"🎯 SINAL COMPRA CNN1D: {data_atual} | Preço: {preco_atual:.2f} | Previsão: {preco_previsto:.2f} | Razão: {razao_preco:.4f}")
            ...
            saida_previsao = razao_preco < 0.99
            ...
            if stop_loss or take_profit or saida_previsao or saida_tempo:
                ...
                if trades_executados <= 10:  # Limitar prints
                    print(f"📉 SINAL VENDA CNN1D ({motivo}): {data_atual} | Preço: {preco_atual:.2f}")
        ...
        if sinal == "COMPRAR":
            ...
            if capital_para_trade > preco_atual:
                ...
                if trades_executados <= 5:
                    print(f"💰 COMPRA EXECUTADA CNN1D: {data_atual} | Preço: {preco_atual:.2f} | Capital: R$ {capital_para_trade:.2f}")
        ...
        # EXECUTAR VENDA
            ...
            if trades_executados <= 10:
                print(f"💸 VENDA EXECUTADA CNN1D: {data_atual} | Preço: {preco_atual:.2f} | Retorno: {retorno_porcentagem:+.2f}%")
    ...
    if trade_ativo and len(datas) > 0:
        ...
        if ultima_data in df.index:
            ...
            print(f"🔚 FECHAMENTO FORÇADO CNN1D: {ultima_data} | Retorno: {retorno_porcentagem:+.2f}%")
    
    print(f"\n🔍 RESUMO BACKTEST CNN1D:")
...
# ===================== VALIDAR PREVISÕES =====================
def ValidarPrevisoes(valores_reais, previsoes, datas_teste, nome_modelo="CNN1D"):
    plt.figure(figsize=(12, 6))
    plt.plot(datas_teste, valores_reais, label='Valor Real', linewidth=2, color='blue')
    plt.plot(datas_teste, previsoes, label=f'Previsão {nome_modelo}', linewidth=1, alpha=0.7, color='red')
    plt.title(f'Comparação: Preço Real vs Previsão {nome_modelo} ({TICKER})')
    plt.legend()
    ...
    print(f"📊 VALIDAÇÃO DAS PREVISÕES {nome_modelo}:")
...
# ===================== EXECUTOR CNN1D =====================
def ExecutarCNN1D():
    print(f"\n🎯 INICIANDO MODELO CNN1D PARA {TICKER}")
    print("=" * 60)
    ...
    # Validar previsões
    ValidarPrevisoes(valores_reais, previsoes, datas_teste, "CNN1D")

    # Backtest com taxas
    print(f"\n💼 Executando backtest CNN1D com gestão de risco...")
    ...
    print(f"\n💰 ========== RESULTADOS BACKTEST COM TAXAS (CNN1D) ==========")
    ...
    # Gráfico final
    plt.figure(figsize=(12, 6))
    plt.plot(curva_patrimonio, label='CNN1D com Gestão de Risco', linewidth=2, color='orange')
    ...
    plt.title(f'Desempenho Estratégia CNN1D vs Buy & Hold ({TICKER})')
...
def CompararModelos(resultados_cnn, resultados_lstm):
    ...
    print(f"\n🏆 ========== COMPARAÇÃO CNN1D vs LSTM ==========")
    print(f"{'Métrica':<20} {'CNN1D':<12} {'LSTM':<12} {'Vencedor':<10}")
    print("-" * 55)
    ...
    print(f"{'Retorno (%)':<20} {retorno_cnn:<12.2f} {retorno_lstm:<12.2f} {vencedor_retorno:<10}")
    ...
    print(f"{'R²':<20} {r2_cnn:<12.4f} {r2_lstm:<12.4f} {vencedor_r2:<10}")
    ...
    print(f"{'MAE (R$)':<20} {mae_cnn:<12.2f} {mae_lstm:<12.2f} {vencedor_mae:<10}")
    ...
    print(f"{'Taxa Acerto (%)':<20} {acerto_cnn:<12.1f} {acerto_lstm:<12.1f} {vencedor_acerto:<10}")
    
    # Número de Trades
    trades_cnn = resultados_cnn['trades']['total_trades']
    trades_lstm = resultados_lstm['trades']['total_trades']
    print(f"{'Nº Trades':<20} {trades_cnn:<12} {trades_lstm:<12} {'-'*10}")
...
if __name__ == "__main__":
    ...
    print("🚀 INICIANDO EXECUÇÃO DO MODELO CNN1D")
    print("=" * 70)
    
    # Executar CNN1D
    resultados_cnn = ExecutarCNN1D()
    
    if resultados_cnn:
        print(f"\n🎉 ✅ Execução CNN1D concluída com sucesso!")
        print(f"📈 Retorno CNN1D: {resultados_cnn['retorno_total']:+.2f}%")
        print(f"📊 Retorno Buy&Hold: {resultados_cnn['retorno_buy_hold']:+.2f}%")
        print(f"🎯 Taxa de Acerto: {resultados_cnn['trades']['taxa_acerto']:.1f}%")
        print(f"💰 Capital Final: R$ {resultados_cnn['capital_final']:.2f}")
        
        # Perguntar se quer comparar com LSTM
        comparar = input("\n🔍 Deseja executar o modelo LSTM para comparação? (s/n): ")
        if comparar.lower() == 's':
            from lstm import ExecutarLSTM  # Importar do código LSTM anterior
            resultados_lstm = ExecutarLSTM()
            if resultados_lstm:
                CompararModelos(resultados_cnn, resultados_lstm)
    else:
        print(f"\n💥 ❌ Erro na execução CNN1D")