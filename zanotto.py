# =========================================================
# 🧠 MODELO LSTM DE PREVISÃO E BACKTESTING (ZANOTTO + MELHORADO)
# =========================================================
...
class LSTMTrader:
    ...
    def create_sequences(self):
        ...
        for i in range(self.window_size, len(scaled) - 1):
            ...
            y.append(scaled[i + 1, 0])  # Preço futuro (Close[t+1])
        return np.array(X), np.array(y)

    def train_test_split(self, X, y, test_size=0.2):
        split = int(len(X) * (1 - test_size))
        self.train_dates = self.data.index[self.window_size:split + self.window_size]
        self.test_dates = self.data.index[split + self.window_size + 1:]
    ...
    def predict(self, X_train, X_test, y_train, y_test):
        ...
        return inverse(y_train_pred), inverse(y_test_pred), inverse(y_train), inverse(y_test)

    def generate_signals(self, preds, actuals):
        """Sinais agressivos: compra em tendência forte e venda em reversão."""
        signals = []
        position = 0
        for i in range(2, len(preds)):
            delta = preds[i] - preds[i - 1]
            prev_delta = preds[i - 1] - preds[i - 2]
            if position == 0 and delta > 0 and prev_delta > 0:
                signals.append('BUY'); position = 1
            elif position == 1 and delta < 0:
    ...
    def evaluate_and_plot(self, X_train, X_test, y_train, y_test):
        ...
        # 1️⃣ Preços reais vs previstos (com setas)
        plt.subplot(3, 2, 1)
        ...
        plt.scatter(np.array(self.test_dates)[buy_idx], np.array(y_test_inv)[buy_idx], marker='^', color='green', s=100, label='Compra')
        plt.scatter(np.array(self.test_dates)[sell_idx], np.array(y_test_inv)[sell_idx], marker='v', color='red', s=100, label='Venda')
